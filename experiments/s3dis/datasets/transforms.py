...
Retained entry points:
    augment_seg()     — segmentation transform shared by slices and points
    pointwolf_seg()   — optional local non-linear distortion helper
...
def augment_seg(slices: np.ndarray, pts_features: np.ndarray,
    ...
    if C >= 6:
        # Rotate vector-valued channels only when channels 3:6 are not RGB.
