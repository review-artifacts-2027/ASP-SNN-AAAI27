...
class DGCNNSegTeacher(nn.Module):
    """Per-point segmentation teacher using dynamic EdgeConv (Wang et
    al., 'Dynamic Graph CNN for Learning on Point Clouds', TOG 2019),
    used as an alternative to PointNetSegTeacher for knowledge
    distillation. Unlike PointNet's independent per-point MLP with a
    single global max-pool, DGCNN recomputes a k-NN graph in feature
    space at every layer, giving the teacher access to local surface
    geometry (edges, curvature) that a purely per-point encoder lacks.
    Selected via `cfg.teacher_arch: dgcnn` (default: 'pointnet').
    """

    def __init__(self, num_classes: int, in_channels: int = 7, k: int = 16):
        super().__init__()
        self.k = k
        self.conv1 = nn.Sequential(
            nn.Conv2d(in_channels * 2, 64, 1), nn.BatchNorm2d(64),
            nn.LeakyReLU(0.2),
        )
        self.conv2 = nn.Sequential(
            nn.Conv2d(64 * 2, 64, 1), nn.BatchNorm2d(64), nn.LeakyReLU(0.2),
        )
        self.conv3 = nn.Sequential(
            nn.Conv2d(64 * 2, 128, 1), nn.BatchNorm2d(128), nn.LeakyReLU(0.2),
        )
        self.conv4 = nn.Sequential(
            nn.Conv1d(64 + 64 + 128, 512, 1), nn.BatchNorm1d(512),
            nn.LeakyReLU(0.2),
        )
        self.seg_head = nn.Sequential(
            nn.Conv1d(512 + 64 + 64 + 128, 256, 1), nn.BatchNorm1d(256),
    ...
    def forward(self, pts_feat):  # [B, N, C]
        x = pts_feat.transpose(2, 1)               # [B, C, N]
        x1 = self.conv1(self._graph_feature(x, self.k)).max(dim=-1)[0]
        x2 = self.conv2(self._graph_feature(x1, self.k)).max(dim=-1)[0]
        x3 = self.conv3(self._graph_feature(x2, self.k)).max(dim=-1)[0]
        x_cat = torch.cat((x1, x2, x3), dim=1)
        global_feat = self.conv4(x_cat).max(dim=-1, keepdim=True)[0]
        global_feat = global_feat.repeat(1, 1, x.shape[-1])
        x_all = torch.cat((global_feat, x1, x2, x3), dim=1)
        return self.seg_head(x_all).transpose(2, 1)  # [B, N, num_classes]
...
def main():
    ...
    # Rare classes (e.g. beam, column) are exempt from the weight-
    # annealing schedule below: a flat, un-annealed inverse-frequency
    # weight let them collapse toward 0% IoU under small-batch gradient
    # oscillation, but annealing them down along with the well-populated
    # classes removes exactly the signal that keeps them learnable.
    rare_class_ids = [
        c for c in getattr(cfg, 'rare_classes', [])
        if class_weights is not None and 0 <= c < class_weights.numel()
    ]
    if rare_class_ids:
        print(f"[Weights] rare classes exempt from annealing: "
              f"{rare_class_ids}")
    ...
    teacher_arch = getattr(cfg, 'teacher_arch', 'pointnet')
    teacher_ckpt_name = 's3dis_kd_teacher.pt' if teacher_arch == 'pointnet' \
        else f's3dis_kd_teacher_{teacher_arch}.pt'
    teacher_ckpt = os.path.join(cfg.ckpt_dir, teacher_ckpt_name)

    if kd_teacher_epochs > 0:
        if teacher_arch == 'dgcnn':
            kd_teacher = DGCNNSegTeacher(
                num_classes=NUM_CLASSES, in_channels=in_ch,
                k=int(getattr(cfg, 'teacher_dgcnn_k', 16)),
            ).to(device)
        else:
            kd_teacher = PointNetSegTeacher(
                num_classes=NUM_CLASSES, in_channels=in_ch,
            ).to(device)
        print(f"[KD] Teacher architecture: {teacher_arch}")
    ...
    lovasz_lambda_base = float(getattr(cfg, 'lovasz_lambda', 1.0))
    lovasz_criterion = LovaszSoftmaxLoss(
        ignore_index=-1, classes="present",
    ) if use_lovasz else None
    if use_lovasz:
        _lz_floor = getattr(cfg, 'lovasz_floor_scale', 1.0)
        print(f"Lovász-Softmax loss: enabled (λ={lovasz_lambda_base}, "
              f"annealed floor λ={lovasz_lambda_base * _lz_floor:.3f} "
              f"from epoch {getattr(cfg, 'cw_anneal_start_epoch', cfg.epochs)})")
    ...
    for epoch in range(start_epoch, cfg.epochs):
        ...
        # ── Annealed class-weight / Lovász schedule ─────────────────────
        # Ramps class weights and the Lovász coefficient DOWN gradually
        # from epoch `cw_anneal_start_epoch` over `cw_anneal_epochs`,
        # settling at `cw_floor_scale` / `lovasz_floor_scale` (not zero).
        # A hard on/off switch reduces small-batch gradient oscillation
        # but also removes the model's incentive to predict rare classes
        # at all; a gradual, non-zero floor keeps some of that incentive
        # while still damping the oscillation. Rare classes (see
        # `rare_class_ids` above) are excluded from this schedule
        # entirely and keep their full weight throughout training.
        cw_anneal_start = getattr(cfg, 'cw_anneal_start_epoch', cfg.epochs)
        cw_anneal_len   = max(1, getattr(cfg, 'cw_anneal_epochs', 1))
        cw_floor_scale  = getattr(cfg, 'cw_floor_scale', 1.0)
        lz_floor_scale  = getattr(cfg, 'lovasz_floor_scale', 1.0)
        anneal_t = 0.0 if epoch < cw_anneal_start else \
            min(1.0, (epoch - cw_anneal_start) / cw_anneal_len)

        if class_weights is not None:
            scale = 1.0 - anneal_t * (1.0 - cw_floor_scale)
            eff_weights = 1.0 + (class_weights - 1.0) * scale
            if rare_class_ids:
                eff_weights[rare_class_ids] = class_weights[rare_class_ids]
            criterion.weight = eff_weights

        lovasz_lambda = lovasz_lambda_base * (
            1.0 - anneal_t * (1.0 - lz_floor_scale)
        )
        ...
        for batch_idx, batch_data in enumerate(train_loader):
            ...
            with autocast(device_type=device.type, enabled=cfg.use_amp):
                ...
                # firing_rate_loss_term() (not mean_firing_rate(), which
                # is a plain float used only for logging) so the sparsity
                # penalty actually contributes gradient to the spiking layers.
                if hasattr(model, 'lif_head') and \
                   hasattr(model.lif_head, 'firing_rate_loss_term'):
                    loss = loss + 0.01 * model.lif_head.firing_rate_loss_term()

            # Gradient accumulation: on memory-constrained GPUs where
            # cfg.batch_size must be small, every BatchNorm layer in the
            # model (encoder, LIF cells, segmentation head) would otherwise
            # see very noisy per-batch statistics. Accumulating brings the
            # EFFECTIVE batch size up to
            # cfg.batch_size * cfg.grad_accum_steps without raising peak
            # memory. No-op when grad_accum_steps is left at 1.
