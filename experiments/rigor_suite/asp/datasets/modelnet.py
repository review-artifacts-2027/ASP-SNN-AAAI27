...
class ModelNetDataset(Dataset):
    ...
    def __getitem__(self, i):
        pts = self._load(i)
        # Train: draw from the global (per-worker, per-epoch reseeded) RNG so the
        # augmentation actually varies across epochs. Seeding solely on `i` made
        # every epoch see a byte-identical cloud, i.e. no augmentation at all.
        # Eval/corruption paths stay deterministic in `i` for reproducibility.
        g = None if self.augment else torch.Generator().manual_seed(i * 6151 + 29)
        idx = torch.randperm(pts.shape[0], generator=g)[: self.n_points]
        pts = pts[idx]
        pts = normalize_cloud(pts.unsqueeze(0)).squeeze(0)
        if self.augment:
            import math
            th = torch.rand(1).item() * 2 * math.pi                 # z-rotation
            c, s = math.cos(th), math.sin(th)
            R = torch.tensor([[c, -s, 0.0], [s, c, 0.0], [0.0, 0.0, 1.0]])
            pts = pts @ R.t()
            pts = pts * (0.8 + 0.45 * torch.rand(3))                # anisotropic scale
            pts = pts + (torch.rand(3) - 0.5) * 0.2                 # translation
            pts = pts + torch.randn(pts.shape).clamp(-5, 5) * 0.01  # jitter
        if self.corruption is not None and self.severity > 0:
            cg = torch.Generator().manual_seed(i * 6151 + 29)
            pts = self.corruption(pts, self.severity, cg)
            pts = normalize_cloud(pts.unsqueeze(0)).squeeze(0)
